...
def periph(fn, data, _led, mode, timeout):
    ...
    if mode is 0: #modeが1 #センサとして経路構築
        data = str(data)
        print(data)
    ...
    if mode is 1: #modeが1 #センサとして経路構築
        data = str(data)
        print(data)
    ...
    if mode is 3:
        ...
        if b._check is False and flag == 0:
            ...
            while b._check is False and timeout > 0:
                dist = gapname + '_' + data + '_' + str(break_dev)
                print(data)
                i = (i + 1) % 10
                _led.on()
                b.set_dev_name(dist, notify=i == 0, indicate=False)
...
if __name__ == "__main__":
    ...
    timer = 10
    green_pin = 14
    _led = machine.Pin(green_pin, machine.Pin.OUT)
    fn = 'info/SN02.json'
    
    mode = 0
    
    periph(fn, data, _led, mode,  timer)
