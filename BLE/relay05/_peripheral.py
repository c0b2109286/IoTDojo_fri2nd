...
def periph(fn, data, _led, mode, timeout):
    ...
    jf_load = ujson.loads(open('info/DN05.json').read())
    ...
    if mode is 0: #communication between senser device
        print(data)
        b._payload_1(str(jf_load["packet_routeTSS"]))
        print(timeout)
    ...
    if mode is 1:
        b._payload_1(str(jf_load["packet_routeTSSB"]))
        print(timeout)
        print(b._connect_count)
        print(data)
        while timeout > 1 or b._connect_count is 0:
    ...
    if mode is 2:
        ...
        packet_name = jf_load["relay01"]
        
        b._payload_1(str(packet_name))
        print(timeout)
        print(b._connect_count)
        while timeout > 1 or b._connect_count is 0:
            #if b._check is False and b._connect_count is 0:
    ...
    if mode is 4:
        print("mode4です")
        print(data)
        b._payload_1(str(jf_load["packet_tosenser2"]))
        print(timeout)
        print(b._connect_count)
        while timeout > 1 or b._connect_count is 0:
            #if b._check is False and b._connect_count is 0:
    ...
    if mode is 7 or mode is 8:
        data = str(routedata)
        print(data)
        b._payload_1(jf_load["packet_call_s"])
        print(timeout)
        print(b._connect_count)
        while timeout > 1 or b._connect_count is 0:
            #if b._check is False and b._connect_count is 0:
    ...
    if mode is 6:
        ...
        jopen = open('data/Routeinfo.json', 'r')
        route = ujson.load(jopen)
